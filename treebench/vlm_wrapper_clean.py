...
class VLMWrapper:
    ...
    @torch.no_grad()
    def score_assistant_continuation(
        self,
        image: Image.Image,
        user_prompt: str,
        assistant_prefix: str,
        ...
        prefix_inputs = self._prepare_inputs(
            image=image,
            user_prompt=user_prompt,
            assistant_prefix=assistant_prefix,
        )
        full_inputs = self._prepare_inputs(
            image=image,
            user_prompt=user_prompt,
            assistant_prefix=assistant_prefix + continuation,
        )
        ...
        if not torch.isfinite(logits).all():
            raise RuntimeError("Non-finite logits in score_assistant_continuation")
